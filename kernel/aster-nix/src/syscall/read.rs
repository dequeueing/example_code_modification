...
use crate::{
    fs::file_table::FileDescripter, log_syscall_entry, prelude::*, proxy::fs::{proxy_sys_read, proxy_sys_write}, util::write_bytes_to_user
};
...
pub fn sys_read(fd: FileDescripter, user_buf_addr: Vaddr, buf_len: usize) -> Result<SyscallReturn> {
    ...
    let ret = proxy_sys_read(fd as usize, &mut read_buf);
